...
class CompleteEfficiencyRatingPredictor(Predictor):
    ...
    def predict(self, team: str, opponent: str) -> Prediction:
        ...
        team_two_point_shot_selection = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._two_sel, team, self.rating.defense._efficiency._shooting._two_sel, opponent)
        team_three_point_shot_percentage = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._three_pct, team, self.rating.defense._efficiency._shooting._three_pct, opponent)
        team_three_point_shot_selection = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._three_sel, team, self.rating.defense._efficiency._shooting._three_sel, opponent)
        team_shooting_rating = (team_two_point_shot_percentage * team_two_point_shot_selection * 2 + team_three_point_shot_percentage * team_three_point_shot_selection * 3) / (team_two_point_shot_selection + team_three_point_shot_selection)

        team_field_goal_shot_percentage = self._calculate_matchup_value(self.rating.offense._efficiency._rebounds._shot_pct, team, self.rating.defense._efficiency._rebounds._shot_pct, opponent)
        ...
        if hasattr(self.rating.offense._efficiency, "_points_per_poss"):
            ...
            team_offensive_rating = ((team_shooting_rating / (1 - team_offensive_rebounds_rating) * (1 - team_turnovers_per_possession) + team_free_throw_rating) * team_points_per_possession) ** 0.5
        else:
            team_offensive_rating = (team_shooting_rating / (1 - team_offensive_rebounds_rating) * (1 - team_turnovers_per_possession) + team_free_throw_rating)

        opponent_two_point_shot_percentage = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._two_pct, opponent, self.rating.defense._efficiency._shooting._two_pct, team)
        opponent_two_point_shot_selection = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._two_sel, opponent, self.rating.defense._efficiency._shooting._two_sel, team)
        opponent_three_point_shot_percentage = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._three_pct, opponent, self.rating.defense._efficiency._shooting._three_pct, team)
        opponent_three_point_shot_selection = self._calculate_matchup_value(self.rating.offense._efficiency._shooting._three_sel, opponent, self.rating.defense._efficiency._shooting._three_sel, team)
        opponent_shooting_rating = (opponent_two_point_shot_percentage * opponent_two_point_shot_selection * 2 + opponent_three_point_shot_percentage * opponent_three_point_shot_selection * 3) / (opponent_two_point_shot_selection + opponent_three_point_shot_selection)

        opponent_field_goal_shot_percentage = self._calculate_matchup_value(self.rating.offense._efficiency._rebounds._shot_pct, opponent, self.rating.defense._efficiency._rebounds._shot_pct, team)
        ...
        if hasattr(self.rating.offense._efficiency, "_points_per_poss"):
            ...
            opponent_offensive_rating = ((opponent_shooting_rating / (1 - opponent_offensive_rebounds_rating) * (1 - opponent_turnovers_per_possession) + opponent_free_throw_rating) * opponent_points_per_possession) ** 0.5
        else:
            opponent_offensive_rating = (opponent_shooting_rating / (1 - opponent_offensive_rebounds_rating) * (1 - opponent_turnovers_per_possession) + opponent_free_throw_rating)
    ...
    def _calculate_matchup_value(cls, team_rating: Rating, team: str, opponent_rating: Rating, opponent: str) -> float:
        ...
        stdev = math.sqrt(pow(opponent_rating._stdevs.get(opponent), 2) + pow(team_rating._stdevs.get(team), 2)) / 2
